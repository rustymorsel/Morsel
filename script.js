/* Morsel — single-file JS app (static site, GitHub Pages friendly)
   ...
   - Tabs: History / Ingredients / Method / Variations / Ratings + Comments
   - Metric/Imperial toggle
   - Finder: what you can make with ingredients you have
   - Favourites + My Recipes
   - Shopping list
   - Seeds 1000s of recipes (procedural) on first run
*/

const LS = {
  UNITS: "morsel_units",
  AGE_OK: "morsel_age_ok",
  RECIPES: "morsel_recipes_v1",
  MY_RECIPES: "morsel_my_recipes_v1",
  FAVS: "morsel_favs_v1",
  COMMENTS: "morsel_comments_v1",
  RATINGS: "morsel_ratings_v1",
  SHOP: "morsel_shop_v1",
  FINDER: "morsel_finder_v1",
};
...
let recipes = [];    // seeded catalogue (food + cocktails)
let myRecipes = [];  // user created
let favs = new Set(JSON.parse(localStorage.getItem(LS.FAVS) || "[]"));
let commentsById = JSON.parse(localStorage.getItem(LS.COMMENTS) || "{}"); // {id:[{t,txt,when}]}
let ratingsById = JSON.parse(localStorage.getItem(LS.RATINGS) || "{}");   // {id:[1..5]}
let shop = JSON.parse(localStorage.getItem(LS.SHOP) || "[]");            // [{txt,done}]
let finder = JSON.parse(localStorage.getItem(LS.FINDER) || "[]");        // ["onion","chicken"]
...
function loadData() {
  ...
  // If no catalogue, seed once
  if (!Array.isArray(recipes) || recipes.length < 50) {
    recipes = buildSeedCatalogue(1200, 220); // 1200 food + 220 cocktails
...
function bindTopActions() {
  ...
  $("#seedBtn").addEventListener("click", () => {
    // Rebuild demo catalogue, but keep user's own recipes
    recipes = buildSeedCatalogue(1200, 220);
    localStorage.setItem(LS.RECIPES, JSON.stringify(recipes));
    renderAll();
...
function bindNav() {
  $$(".navbtn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Cocktails tab is special (age gate)
...
function showView(id, updateNav=false) {
  ...
  // Render on demand for freshness
...
function renderRecipes() {
  ...
  const filtered = base.filter(r => {
    ...
    // allergen filter (include recipes that contain that allergen tag)
    if (allergen && !r.allergens.includes(allergen)) return false;
    // mood filter
    if (mood && !r.moods.includes(mood)) return false;

    // search across name, moods, ingredients, allergens
    if (!q) return true;
    const hay = [
      r.name,
      r.moods.join(" "),
...
function renderFinder() {
  ...
  const tolerance = parseInt($("#tolerance").value, 10); // percent of missing allowed
...
function renderShop() {
  ...
  ul.querySelectorAll(".shopitem").forEach(li => {
    li.addEventListener("click", (e) => {
      ...
      if (x !== null) return; // handled below
...
function saveNewRecipe() {
  ...
  const type = $("#addType").value; // food/cocktail
  ...
  if (type === "cocktail" && !allergens.includes("Alcohol")) {
    allergens.push("Alcohol");
  }
...
function wireCardEvents(container) {
  ...
  container.querySelectorAll("[data-add-shop]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // keep the line text as shopping items
      lines.slice(0, 40).reverse().forEach(line => {
        shop.unshift({ txt: line, done: false });
      });
      saveShop();
  ...
  container.querySelectorAll("[data-rate]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // keep last 200 ratings for sanity
  ...
  container.querySelectorAll("[data-comment-send]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // keep last 60 comments per recipe
...
function extractIngredientWords(text) {
  // crude ingredient keyword extraction for Finder
  ...
  for (const line of lines) {
    const w = line
      ...
      .split(/\s+/)
      .filter(x => x.length >= 3);
    // ignore common cooking words
    const stop = new Set(["fresh","taste","small","large","medium","cup","cups","tbsp","tsp","teaspoon","tablespoon","grams","gram","kg","ml","litre","liter","pinch","dash","chopped","sliced","diced","minced","optional"]);
    w.forEach(x => { if (!stop.has(x)) words.push(x); });
  }
  return uniqueWords(words);
}

/* -------------------- Units (simple conversion) -------------------- */
/* We store ingredient lines as free text.
   If line starts with a number + metric unit, we try to convert.
   Supported:
   g -> oz, kg -> lb
   ml -> fl oz, l -> cups (approx) OR fl oz
*/
function formatIngredientsForUnits(text) {
  if (units === "metric") return text || "";
  const lines = (text || "").split("\n");
  const out = lines.map(line => convertLineToImperial(line));
  return out.join("\n");
}

function convertLineToImperial(line) {
  const s = line.trim();
  // patterns: "200 g chicken", "1.5 l stock", "250 ml milk", "2 kg beef"
  ...
  if (unit === "g") {
    ...
    converted = `${roundNice(oz)} oz`;
  } else if (unit === "kg") {
    const lb = qty * 2.20462;
    converted = `${roundNice(lb)} lb`;
  } else if (unit === "ml") {
    const floz = qty * 0.033814;
    converted = `${roundNice(floz)} fl oz`;
  } else if (unit === "l") {
    const floz = qty * 33.814;
    converted = `${roundNice(floz)} fl oz`;
  } else {
    return line;
  }
...
function buildSeedCatalogue(foodCount = 1000, cocktailCount = 200) {
  ...
  for (let i=0; i<foodCount; i++) {
    ...
    const allergens = pickAllergens(allergensPool, i, 0.18); // 18% have 1 allergen tag
    ...
    const history = `Born from a ${vibe.toLowerCase()} kitchen idea: make a solid meal with what’s around, keep it honest, feed people well.`;
    const method = [
  ...
  for (let i=0; i<cocktailCount; i++) {
    ...
    const level = ((i % 3) + 1); // keep cocktails simpler
    const ingredients = [
...
function pickAllergens(pool, i, chance=0.15) {
  // deterministic based on index: not true random so catalogue is stable
